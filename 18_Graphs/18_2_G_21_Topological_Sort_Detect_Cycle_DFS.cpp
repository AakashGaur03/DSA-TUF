...
bool dfs(int node, stack<int> &st, vector<vector<int>> &adj,
  ...
  for (auto adjNode : adj[node]) {
    if (!vis[adjNode]) {
      if (dfs(adjNode, st, adj, pathVis, vis)) {
        return true;
      }
...
bool topoSortCycleDetect(int V, vector<vector<int>> &adj) {
  // TC O( V + E )
  // SC O( 2N )
  vector<int> vis(V, 0);
...
int main() {
  cout << "18 G 21 Topological Sort Cycle Detection DFS" << endl;
  ...
  bool hasCycle = topoSortCycleDetect(V, adj);

  if (hasCycle) {
    cout << "Cycle detected in graph\n";
  } else {
    cout << "No cycle detected (Graph is DAG)\n";
  }
  return 0;
}