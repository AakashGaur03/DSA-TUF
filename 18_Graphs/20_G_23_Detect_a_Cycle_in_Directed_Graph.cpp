...
bool topoSortDetectCycle(int V, vector<int> adj[]) {
  // TC(V+E)
  // SC(2V)
  ...
  // For Clarity Kept it same we can instead of storing Topo Sort Just simply Do
  // a counter Variable and coutner++ instead of topo.push_back
  if (topo.size() == V) {
    return false;
  }
  return true;
...
int main() {
  cout << "20 G 23 Detect a Cycle in Directed Graph" << endl;
