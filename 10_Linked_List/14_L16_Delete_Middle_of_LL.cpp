...
Node *optimizedDeleteMiddleLL(Node *head) {
  // TC O(N/2)
  ...
  Node *slow = head;
  Node *fast = head;
  fast = head->next->next; // After this we will land slow on middle - 1
  // Means we will keep Fast at 1 ahead so Slow misses one step
...
int main() {
  ...
  printLL(head);
  head2 = deleteMiddleLL(head2);
  printLL(head2);
  head = optimizedDeleteMiddleLL(head);
  printLL(head);
  head2 = optimizedDeleteMiddleLL(head2);
  printLL(head2);
